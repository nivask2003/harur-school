<?php include "function.php" ?>
<?php template_header("Management & Leadership", "Explore the Management & Leadership team of Harur International School, dedicated to educational quality, student welfare, institutional development, strong governance, and continuous improvement."); ?>
<section class="page-banner" id="management">
    <h1>Management & Leadership</h1>
</section>
<section class="management-1">
    <div class="container">
        <span class="badge" data-aos="fade-right" data-aos-duration="800">MANAGEMENT & LEADERSHIP</span>
        <h2 class="section-title" data-aos="fade-left" data-aos-duration="800" data-aos-delay="100">Leading With Purpose, Inspiring</h2>
        <p class="section-desc" data-aos="fade-up" data-aos-duration="800" data-aos-delay="200">The Management Team of Harur International School provides responsible and forward-looking leadership focused on educational quality, student welfare, institutional development, and strong governance. Working in partnership with the Principal, teachers, parents, and the wider community, the team supports continuous improvement and ensures that the school's vision and values are reflected in its academic and co-curricular programmes.</p>
        <table class="table" data-aos="fade-up" data-aos-duration="800" data-aos-delay="300">
            <thead>
                <tr>
                    <th>No.</th>
                    <th>Name</th>
                    <th>Designation</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td>1</td>
                    <td>Mr. G. Kailasam</td>
                    <td>Chairman</td>
                </tr>
                <tr>
                    <td>2</td>
                    <td>Mr. K. Rangasamy</td>
                    <td>Secretary</td>
                </tr>
                <tr>
                    <td>3</td>
                    <td>Mr. G. Thennarasu</td>
                    <td>Treasurer</td>
                </tr>
                <tr>
                    <td>4</td>
                    <td>Prof. J. Shanmugam</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>5</td>
                    <td>Mrs. K. Sailaja</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>6</td>
                    <td>Mr. R. Srinivethan</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>7</td>
                    <td>Mr. R. Kaushik</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>8</td>
                    <td>Mrs. R. Vennila</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>9</td>
                    <td>Mr. C. Kandasamy</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>10</td>
                    <td>Mr. G. Sampath</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>11</td>
                    <td>Mr. N. Arunprasath</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>12</td>
                    <td>Mr. A. Sivakumar</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>13</td>
                    <td>Mrs. R. Manimegalai</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>14</td>
                    <td>Mr. K. Nedunchelian</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>15</td>
                    <td>Mrs. S. Agalya</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>16</td>
                    <td>Mr. S. Sivakumar</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>17</td>
                    <td>Mr. P. V. Krishnan</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>18</td>
                    <td>Mr. S. Theertharasu</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>19</td>
                    <td>Mr. V. Jayaraman</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>20</td>
                    <td>Mr. M. P. Lakshmanan</td>
                    <td>Trustee</td>
                </tr>
                <tr>
                    <td>21</td>
                    <td>Mr. K. Rathinasamy</td>
                    <td>Trustee</td>
                </tr>
            </tbody>
        </table>

    </div>
</section>
<?php template_footer() ?>